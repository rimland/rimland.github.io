...
考虑到 Podman 不使用守护进程来开发、管理和运行 OCI 容器，因此它必须在 Linux OS 上运行。 容器能以root 模式运行，也能以非 root 运行模式运行。 Docker 利用一个守护进程（该守护进程是一个持久的后台进程）来处理主机上所有容器的管理职责。 Docker 依赖于客户端/服务端（C/S）架构，守护进程扮演着服务端的角色，而客户端通过命令行界面（CLI）进行通信。
