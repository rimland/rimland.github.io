...
ASP.NET Core 支持依赖关系注入（DI）软件设计模式，该模式允许我们注册服务、控制如何实例化这些服务并将其注入到不同的组件中。一些服务可以在短周期内实例化，并且仅在特定的组件和请求中可用；一些实例仅被实例化一次，并在整个应用程序生命周期内可用。这就是 ASP.NET Core 中可用的服务生命周期。
...
每次请求会创建一次服务实例。参与处理单个请求的所有中间件、MVC 控制器等等，都将获得相同的实例。实体框架上下文（Entity Framework context）是 *Scoped* 服务的一个很好的候选者。我们可以使用 **AddScoped** 方法注册 *Scoped* 服务，如下所示：
...
每次请求 *Transient* 生命期服务时都会创建它们。此生命周期最适合轻量级、无状态服务。我们可以使用 **AddTransient** 方法注册 *Transient* 服务，如下所示：
