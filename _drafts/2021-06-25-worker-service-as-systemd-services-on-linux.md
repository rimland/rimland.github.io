...
`journalctl` 命令可以用来查看 systemd 收集的日志。*systemd-journald* 服务负责 systemd 的日志收集，它从内核、systemd 服务和其他源检索信息。日志被集中收集，便于对其进行检索查询。journal 中的日志记录是结构化和有索引的，因此 `journalctl` 能够以各种有益的格式来展现日志信息。
