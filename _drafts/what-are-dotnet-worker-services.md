...
<!-- Worker类是新的东西，您不会在默认的ASP.NET Core项目模板中找到。 这就是托管服务与主机结合在一起的魔力，它是工作者服务的基础。

“Worker”类是在默认的ASP.NET核心项目模板中找不到的新类。在这里，托管服务的魔力与主机相结合，提供了辅助服务的基础。

Worker类是你在默认ASP中找不到的新类。
NET Core项目模板。
这就是托管服务与主机相结合的魔力所在，它提供了一个工作者服务的基础。 -->
...
在本文中，我介绍了 Worker Service 项目模板，以及它的一些潜在用例。我们探索了使用 Worker Service 模板创建的新的项目中所包含的三个默认文件。
