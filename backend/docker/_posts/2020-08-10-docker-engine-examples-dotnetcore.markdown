...
本示例演示了怎样容器化一个 ASP.NET Core 应用程序。

## 为什么构建 ASP.NET Core？

- [开放源码](https://github.com/aspnet/home)
- 开发和运行您的 ASP.NET Core 应用在 Windows、MacOS、和 Linux 上可跨平台
- 非常适合现代基于云的应用程序，如 web 应用程序、物联网（IoT）应用程序和移动后端
- ASP.NET Core 应用程序可以运行在 [.NET Core](https://www.microsoft.com/net/core/platform) 或完整的 [.NET Framework](https://www.microsoft.com/net/framework) 框架上
- 为部署到云端或在本地运行的应用程序精心设计以提供优化的开发框架
- 开销最小的模块化组件在构建解决方案时保持了灵活性

## 前提条件

此示例假定在您的机器已经有一个 ASP.NET Core 应用程序。如果您是 ASP.NET 新手，您可以按照一个[简单的教程](https://www.asp.net/get-started)初始化一个项目或者克隆(clone) [ASP.NET Docker Sample](https://github.com/dotnet/dotnet-docker/tree/master/samples/aspnetapp)。

## 为 ASP.NET Core 应用程序创建一个 Dockerfile
1. 在项目文件夹中创建一个 `Dockerfile`。
2. 为 Linux 或 [Windows 容器](https://docs.microsoft.com/virtualization/windowscontainers/about/) 将下面的文本添加到 `Dockerfile` 中。下面的标签是多架构(multi-arch)的，这意味着它们根据 [Docker Desktop for Windows](https://docs.docker.com/docker-for-windows/) 中设置的模式来拉取 Windows 或 Linux 容器。阅读更多关于[在 Windows 和 Linux 容器间切换](https://docs.docker.com/docker-for-windows/#switch-between-windows-and-linux-containers)。
3. `Dockerfile` 假定您的应用程序名为 `aspnetapp`。更改 `Dockerfile` 以使用项目的 DLL 文件。
...
4. 为了使构建上下文尽可能小，添加一个 [`.dockerignore` 文件](https://docs.docker.com/engine/reference/builder/#dockerignore-file) 到您的项目文件夹，并将以下内容复制到其中。
...
> 链接 ： [英文原文](https://docs.docker.com/engine/examples/dotnetcore/)
